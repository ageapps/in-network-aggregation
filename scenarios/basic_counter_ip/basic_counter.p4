...
const bit<8> PROTO_pcounter = 0xFD;
...
control MyEgress(inout headers hdr,
    ...
    action setCounter() {
        ...
        hdr.pcounter.pid = hdr.ipv4.protocol;
        hdr.ipv4.protocol = PROTO_pcounter;
        packetCounter.write( (bit<32>)standard_metadata.egress_spec, 0);

    }
    action addCounter() {
        // hdr.pcounter.count = hdr.pcounter.count + 1;
        // get register myCount
        packetCounter.read( tmp, (bit<32>)standard_metadata.egress_spec);        
        // add myCount header
        hdr.pcounter.count = tmp+1;
        // sum to the register
        packetCounter.write((bit<32>)standard_metadata.egress_spec, tmp+1);
    }
